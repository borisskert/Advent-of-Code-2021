...
import Data.Set (toList, fromList, union, empty, insert)

scannerPosition :: Scan -> Vector
scannerPosition (position, _, _) = position

absoluteScannerPositions :: [Scan] -> Vectors
absoluteScannerPositions (scan0 : scans) = (scanner0Position `insert`) . search $ ([scan0], scans)
  where
    scanner0Position = scannerPosition scan0 :: Vector

    search :: ([Scan], [Scan]) -> Vectors
    search (_, []) = empty
    search (scan0 : otherScans, open) = ((fromList . map scannerPosition $ found) `union`) $ search (otherScans ++ found, newOpen)
      where
...
largestManhattanDistance input = maximum . map (uncurry manhattanDistance) $ combinations
  where
    ...
    combinations = (\xs -> [(x,y) | x <- xs, y <- xs, x /= y]) . toList $ scannerPositions :: [(Vector, Vector)]
